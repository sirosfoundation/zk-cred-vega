//! Ported from `microsoft/Nova`'s `src/gadgets/nonnative/bignat.rs` (MIT),
//! itself adapted from `bellman-bignat` (MIT). See `super`'s module doc for
//! what's trimmed relative to upstream (`fold_bn`, `inputize`, and the RO
//! folding-scheme test module — none apply here).
      ...
      .map(|limb_i| {
        cs.alloc(
          ...
          || match values_cell {
            ...
            Err(ref _e) => Err(SynthesisError::AssignmentMissing),
          },
        )
