//! Ported from `microsoft/Nova`'s `src/gadgets/nonnative/util.rs` (MIT),
//! trimmed to drop the second half (`fingerprint_bignat`,
//! `absorb_bignat_in_ro*`) which exists only to serve Nova's own IVC
//! folding transcript — not needed here. See `super`'s module doc.
