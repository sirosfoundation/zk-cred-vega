//! Assembles the MSO's variable-length `valueDigests` section: 4 entries,
//! each `digestID` (1, 2, 3, or 5 CBOR bytes, per `cbor_uint`) immediately
//! followed by a fixed-width tail (`0x58 0x20` bstr header + the 32-byte
//! digest itself), back to back with no padding — matching a real
//! issuer's exact bytes so the whole `Sig_structure`'s SHA-256 comes out
//! byte-identical (see `HANDOFF.md`'s digestID-binding writeup).
//!
...
//! [`crate::onehot_cursor`]. Each subsequent entry's start position
//! depends on the sum of all preceding widths; naively tracking every
//! `(w0,w1,w2,w3)` combination is `4^4 = 256` states by the last entry —
//! `onehot_cursor::convolve_sum` collapses that to the much smaller set of
//! *distinct achievable offsets* (at most 17 by the last entry) by
//! deduplicating combinations that land on the same value.
...
/// Upper bound on the whole section's byte length: every entry at its
/// widest (5-byte digestID + fixed tail).
pub const MAX_SECTION_BYTES: usize = NUM_ENTRIES * (MAX_CBOR_UINT_BYTES + ENTRY_TAIL_LEN);
...
/// (`0x58 0x20` + the 32 digest bytes — build via
/// [`crate::mdoc_core::inputize_bits`]'s sibling allocation pattern, or
/// see this module's own tests for a minimal example).
...
fn alloc_byte_bits<Scalar, CS>(mut cs: CS, bytes: &[u8]) -> Result<Vec<Boolean>, SynthesisError>
where
  Scalar: PrimeField,
...
/// Native reference: the exact bytes [`assemble_digest_id_section`]'s
/// real (non-don't-care) prefix must equal, plus that real length —
/// `tails[i]` must be exactly [`ENTRY_TAIL_LEN`] bytes (`0x58 0x20` + the
/// 32-byte digest).
pub fn native_digest_id_section_bytes(digest_ids: &[u32; NUM_ENTRIES], tails: &[[u8; ENTRY_TAIL_LEN]; NUM_ENTRIES]) -> (Vec<u8>, usize) {
  let mut out = Vec::with_capacity(MAX_SECTION_BYTES);
  for i in 0..NUM_ENTRIES {
    out.extend_from_slice(&cbor_uint::encode_cbor_uint(digest_ids[i]));
    out.extend_from_slice(&tails[i]);
  }
  let real_len = out.len();
  (out, real_len)
}
...
/// Builds the assembled output buffer (`MAX_SECTION_BYTES*8` bits) from
/// `placements`, one output bit at a time: for each bit, `OR`s together
/// every placement's contribution (each already exclusive within itself
/// via one-hot cursors/widths), so unions across *different* placements
/// are safe exactly when their ranges never overlap for any witness — true
/// here because entry `i+1` never starts before entry `i` ends.
fn assemble_from_placements<Scalar, CS>(mut cs: CS, placements: &[Placement]) -> Result<Vec<Boolean>, SynthesisError>
where
  Scalar: PrimeField,
  CS: ConstraintSystem<Scalar>,
{
  let mut out: Vec<Boolean> = Vec::with_capacity(MAX_SECTION_BYTES * 8);
  for p_byte in 0..MAX_SECTION_BYTES {
    for bit_idx in 0..8 {
...
/// Assembles the whole 4-entry `valueDigests` section (see module doc),
/// returning the fixed-size ([`MAX_SECTION_BYTES`]`*8`-bit) buffer and the
/// native real (non-don't-care) length in bytes — the latter feeds
/// `sha256_var_sized`'s `real_len` for whatever wraps this section into
/// the full MSO.
pub fn assemble_digest_id_section<Scalar, CS>(mut cs: CS, entries: &[DigestIdEntry; NUM_ENTRIES]) -> Result<(Vec<Boolean>, usize), SynthesisError>
where
  Scalar: PrimeField,
  CS: ConstraintSystem<Scalar>,
{
  ...
  // `end_i` is where its tail starts (== start_i + real width). `start_0`
  // is the trivial single-value {0} selector (Boolean::constant(true) —
  // no allocation, no convolution needed since there's nothing preceding
  // it to sum with). Every cursor distribution is computed up front into
  // owned, function-lifetime `Vec`s so the `Placement`s built below can
  // borrow from them with ordinary (non-`unsafe`) lifetimes — no
  // `transmute` needed.
  ...
  for i in 0..NUM_ENTRIES {
    let (end_onehot, end_values) = if start_values_list[i].len() == 1 {
      ...
      // start_values_list[i][0] (0 for i==0's start, so end == width
      // verbatim).
  ...
  let mut placements: Vec<Placement> = Vec::with_capacity(NUM_ENTRIES * 2);
  for i in 0..NUM_ENTRIES {
  ...
  let real_len: usize = real_widths.iter().sum::<usize>() + NUM_ENTRIES * ENTRY_TAIL_LEN;
  let assembled = assemble_from_placements::<Scalar, _>(cs.namespace(|| "assemble"), &placements)?;
...
mod tests {
  ...
  fn run_case(digest_ids: [u32; NUM_ENTRIES]) {
    ...
    let (assembled, real_len) = assemble_digest_id_section::<Scalar, _>(cs.namespace(|| "assemble"), &entries).expect("assemble");
    ...
    let (expected_bytes, expected_len) = native_digest_id_section_bytes(&digest_ids, &tails);
    assert_eq!(real_len, expected_len, "digest_ids={digest_ids:?}");
    let got_bytes = bits_to_bytes(&assembled);
    assert_eq!(&got_bytes[..real_len], &expected_bytes[..], "digest_ids={digest_ids:?}");
    assert_eq!(got_bytes.len(), MAX_SECTION_BYTES);
  }
